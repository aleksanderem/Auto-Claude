...
mock_init = MagicMock()
mock_init.init_ouro_dir = MagicMock(return_value=(Path("/tmp"), False))
...
class TestGetSpecsDir:
    ...
    def test_returns_specs_path(self, temp_dir: Path):
        ...
        with patch('spec.pipeline.models.init_ouro_dir') as mock_init:
            mock_init.return_value = (temp_dir / ".ouro", False)

            result = get_specs_dir(temp_dir)

            assert result == temp_dir / ".ouro" / "specs"

    def test_calls_init_ouro_dir(self, temp_dir: Path):
