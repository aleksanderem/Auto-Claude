...
export const createTerminalAPI = (): TerminalAPI => ({
  ...
  invokeClaudeWithSystemPrompt: (id: string, systemPrompt: string, cwd?: string): void =>
    ipcRenderer.send(IPC_CHANNELS.TERMINAL_INVOKE_CLAUDE_WITH_SYSTEM_PROMPT, id, systemPrompt, cwd),
  ...
  runNpmScript: (terminalId: string, scriptName: string): void =>
    ipcRenderer.send(IPC_CHANNELS.TERMINAL_RUN_NPM_SCRIPT, terminalId, scriptName),
