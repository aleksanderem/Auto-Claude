...
function validateStatusTransition(
  ...
  if (newStatus === "human_review") {
    ...
    const hasFailedOrActiveSubtasks = task.subtasks.some(
      (s) => s.status === "failed" || s.status === "in_progress"
    );
    if (!hasApprovedQA && !hasFailedOrActiveSubtasks) {
      console.warn(
        `[validateStatusTransition] Blocking human_review - task ${task.id} has no QA approval and all subtasks pending (phase: ${phase})`
      );
      return false;
    }
  }
      ...
      if (task && project) {
        ...
        if (code === 0) {
          ...
          if (isActiveStatus && hasSubtasks && !hasIncompleteSubtasks && hasApprovedQA) {
            ...
            console.warn(
              `[Task ${taskId}] Fallback: Moving to human_review (process exited successfully, all ${task.subtasks.length} subtasks completed, QA approved)`
            );
          ...
          if (validateStatusTransition(task, "human_review", "failed")) {
            ...
            console.warn(
              `[Task ${taskId}] Process failed but validation blocked human_review - keeping current status (${task.status})`
            );
          }
        }
      }
