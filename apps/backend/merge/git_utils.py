...
def find_worktree(project_dir: Path, task_id: str) -> Path | None:
    ...
    # Legacy fallback for backwards compatibility (.auto-claude)
    legacy_auto_claude_dir = project_dir / ".auto-claude" / "worktrees" / "tasks"
    if legacy_auto_claude_dir.exists():
        for entry in legacy_auto_claude_dir.iterdir():
            if entry.is_dir() and task_id in entry.name:
                return entry

    # Legacy fallback for backwards compatibility (.worktrees)
