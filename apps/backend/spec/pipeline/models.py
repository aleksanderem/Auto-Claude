...
def get_specs_dir(project_dir: Path) -> Path:
    ...
    IMPORTANT: Only .ouro/ is considered an "installed" Ouro.
    The ouro/ folder (if it exists) is SOURCE CODE being developed,
    not an installation. This allows Ouro to be used to develop itself.

    This function also ensures .ouro is added to .gitignore on first use.

    Args:
        project_dir: The project root directory

    Returns:
        Path to the specs directory within .ouro/
    """
    # Initialize .ouro directory and ensure it's in .gitignore
    ouro_dir, _ = init_ouro_dir(project_dir)

    # Return the specs directory path
    return ouro_dir / "specs"
